...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgWithdraw({ value }: msgWithdrawParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.savings.v1beta1.MsgWithdraw", value: MsgWithdraw.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgDeposit({ value }: msgDepositParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.savings.v1beta1.MsgDeposit", value: MsgDeposit.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			JoltifySavingsV1Beta1: new SDKModule(test)
		},
		registry: msgTypes
  }
}
