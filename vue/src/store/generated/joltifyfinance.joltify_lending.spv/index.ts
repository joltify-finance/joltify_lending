	...
	actions: {
		...
		async sendMsgAddInvestors({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.JoltifyfinanceJoltifyLendingSpv.tx.sendMsgAddInvestors({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgAddInvestors({ rootGetters }, { value }) {
			try {
				...
				const msg = await client.JoltifyfinanceJoltifyLendingSpv.tx.msgAddInvestors({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgAddInvestors:Init Could not initialize signing client. Wallet is required.')
