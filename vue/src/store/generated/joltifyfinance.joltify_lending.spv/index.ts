	...
	actions: {
		...
		async sendMsgBorrow({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.JoltifyfinanceJoltifyLendingSpv.tx.sendMsgBorrow({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgBorrow({ rootGetters }, { value }) {
			try {
				...
				const msg = await client.JoltifyfinanceJoltifyLendingSpv.tx.msgBorrow({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgBorrow:Init Could not initialize signing client. Wallet is required.')
