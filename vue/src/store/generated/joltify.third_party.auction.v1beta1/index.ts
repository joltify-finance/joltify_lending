...
import { BaseAuction } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.auction.v1beta1/types"
import { SurplusAuction } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.auction.v1beta1/types"
import { DebtAuction } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.auction.v1beta1/types"
import { CollateralAuction } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.auction.v1beta1/types"
import { WeightedAddresses } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.auction.v1beta1/types"
import { Params } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.auction.v1beta1/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: joltify.third_party.auction.v1beta1 initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.JoltifyThirdPartyAuctionV1Beta1.query.queryParams()).data
		...
		async QueryAuction({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyAuctionV1Beta1.query.queryAuction( key.auction_id)).data
		...
		async QueryAuctions({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyAuctionV1Beta1.query.queryAuctions(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.JoltifyThirdPartyAuctionV1Beta1.query.queryAuctions({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async QueryNextAuctionID({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyAuctionV1Beta1.query.queryNextAuctionId()).data
		...
		async sendMsgPlaceBid({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.JoltifyThirdPartyAuctionV1Beta1.tx.sendMsgPlaceBid({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgPlaceBid({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.JoltifyThirdPartyAuctionV1Beta1.tx.msgPlaceBid({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPlaceBid:Init Could not initialize signing client. Wallet is required.')
