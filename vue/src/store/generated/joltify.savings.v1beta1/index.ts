...
import { Params } from "../ts-client/joltify.savings.v1beta1/types"
import { Deposit } from "../ts-client/joltify.savings.v1beta1/types"


export { Params, Deposit };
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: joltify.savings.v1beta1 initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.JoltifySavingsV1Beta1.query.queryParams()).data
		...
		async QueryDeposits({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifySavingsV1Beta1.query.queryDeposits(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.JoltifySavingsV1Beta1.query.queryDeposits({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgDeposit({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.JoltifySavingsV1Beta1.tx.sendMsgDeposit({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgWithdraw({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.JoltifySavingsV1Beta1.tx.sendMsgWithdraw({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgDeposit({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.JoltifySavingsV1Beta1.tx.msgDeposit({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDeposit:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgWithdraw({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.JoltifySavingsV1Beta1.tx.msgWithdraw({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgWithdraw:Init Could not initialize signing client. Wallet is required.')
