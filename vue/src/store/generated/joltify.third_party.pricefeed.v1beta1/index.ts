...
import { PostedPriceResponse } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
import { CurrentPriceResponse } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
import { MarketResponse } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
import { Params } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
import { Market } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
import { PostedPrice } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
import { CurrentPrice } from "joltify-finance-joltify_lending-client-ts/joltify.third_party.pricefeed.v1beta1/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: joltify.third_party.pricefeed.v1beta1 initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.JoltifyThirdPartyPricefeedV1Beta1.query.queryParams()).data
		...
		async QueryPrice({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyPricefeedV1Beta1.query.queryPrice( key.market_id)).data
		...
		async QueryPrices({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyPricefeedV1Beta1.query.queryPrices()).data
		...
		async QueryRawPrices({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyPricefeedV1Beta1.query.queryRawPrices( key.market_id)).data
		...
		async QueryOracles({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyPricefeedV1Beta1.query.queryOracles( key.market_id)).data
		...
		async QueryMarkets({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.JoltifyThirdPartyPricefeedV1Beta1.query.queryMarkets()).data
		...
		async sendMsgPostPrice({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.JoltifyThirdPartyPricefeedV1Beta1.tx.sendMsgPostPrice({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgPostPrice({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.JoltifyThirdPartyPricefeedV1Beta1.tx.msgPostPrice({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPostPrice:Init Could not initialize signing client. Wallet is required.')
