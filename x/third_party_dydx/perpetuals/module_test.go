...
import (
	...
	testutil_json "github.com/joltify-finance/joltify_lending/testutil/dydx/testutil/json"
	"github.com/joltify-finance/joltify_lending/testutil/dydx/testutil/keeper"
	epochs_keeper "github.com/joltify-finance/joltify_lending/x/third_party_dydx/epochs/keeper"
	epoch_types "github.com/joltify-finance/joltify_lending/x/third_party_dydx/epochs/types"
	"github.com/joltify-finance/joltify_lending/x/third_party_dydx/perpetuals"
	perpetuals_keeper "github.com/joltify-finance/joltify_lending/x/third_party_dydx/perpetuals/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/joltify/third_party/dydxprotocol/perpetuals/perpetual", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/joltify/third_party/dydxprotocol/perpetuals/perpetual/0", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/joltify/third_party/dydxprotocol/perpetuals/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
