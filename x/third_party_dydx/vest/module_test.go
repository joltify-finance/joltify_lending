...
import (
	...
	vest_keeper "github.com/joltify-finance/joltify_lending/x/third_party_dydx/vest/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/joltify/third_party/dydxprotocol/v4/vest/vest_entry", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/joltify/third_party/dydxprotocol/v4/vest/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
