...
import (
	...
	clob_keeper "github.com/joltify-finance/joltify_lending/x/third_party_dydx/clob/keeper"
	"github.com/joltify-finance/joltify_lending/x/third_party_dydx/clob/memclob"
	clob_types "github.com/joltify-finance/joltify_lending/x/third_party_dydx/clob/types"
	"github.com/joltify-finance/joltify_lending/x/third_party_dydx/perpetuals"
	perp_keeper "github.com/joltify-finance/joltify_lending/x/third_party_dydx/perpetuals/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/joltify/third_party/dydxprotocol/clob/clob_pair", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/joltify/third_party/dydxprotocol/clob/clob_pair/0", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
