...
func (k Keeper) LegacyAddOrdersForPruning(ctx sdk.Context, orderIds []types.OrderId, prunableBlockHeight uint32) {
	...
	potentiallyPrunableOrdersSet := make(map[types.OrderId]bool)
	potentiallyPrunableOrders := types.PotentiallyPrunableOrders{}
	potentiallyPrunableOrderIds := make([]types.OrderId, len(orderIds))
	...
	dedupedOrderIds := make([]types.OrderId, 0, len(potentiallyPrunableOrdersSet))
