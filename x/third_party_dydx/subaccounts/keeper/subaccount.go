...
import (
	...
	perplib "github.com/joltify-finance/joltify_lending/x/third_party_dydx/perpetuals/lib"
...
func (k Keeper) getSettledUpdates(
	...
	idToSettledSubaccount := make(map[types.SubaccountId]types.Subaccount)
...
func IsValidStateTransitionForUndercollateralizedSubaccount(
	...
	underCollateralizationResult := types.StillUndercollateralized
...
func applyUpdatesToPositions[
	...
	for _, pos := range positions {
		...
		if !exists {
			...
			newPos := types.NewPositionUpdate(id)

			// Add the position size and update together to get the new size.
			bigNewPositionSize := new(big.Int).Add(
				pos.GetBigQuantums(),
