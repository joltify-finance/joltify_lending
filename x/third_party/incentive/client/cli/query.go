...
func executeUSDXMintingRewardsQuery(cliCtx client.Context, params types2.QueryRewardsParams) (types2.USDXMintingClaims, error) {
	bz, err := cliCtx.LegacyAmino.MarshalJSON(params)
	if err != nil {
		return types2.USDXMintingClaims{}, err
	}

	route := fmt.Sprintf("custom/%s/%s", types2.ModuleName, types2.QueryGetUSDXMintingRewards)
	res, height, err := cliCtx.QueryWithData(route, bz)
	if err != nil {
		return types2.USDXMintingClaims{}, err
	}
	...
	var claims types2.USDXMintingClaims
	if err := cliCtx.LegacyAmino.UnmarshalJSON(res, &claims); err != nil {
		return types2.USDXMintingClaims{}, fmt.Errorf("failed to unmarshal claims: %w", err)
	}

	return claims, nil
}

func executeDelegatorRewardsQuery(cliCtx client.Context, params types2.QueryRewardsParams) (types2.DelegatorClaims, error) {
	bz, err := cliCtx.LegacyAmino.MarshalJSON(params)
	if err != nil {
		return types2.DelegatorClaims{}, err
	}

	route := fmt.Sprintf("custom/%s/%s", types2.ModuleName, types2.QueryGetDelegatorRewards)
	res, height, err := cliCtx.QueryWithData(route, bz)
	if err != nil {
		return types2.DelegatorClaims{}, err
	}
	...
	var claims types2.DelegatorClaims
	if err := cliCtx.LegacyAmino.UnmarshalJSON(res, &claims); err != nil {
		return types2.DelegatorClaims{}, fmt.Errorf("failed to unmarshal claims: %w", err)
	}

	return claims, nil
}

func executeSwapRewardsQuery(cliCtx client.Context, params types2.QueryRewardsParams) (types2.SwapClaims, error) {
	bz, err := cliCtx.LegacyAmino.MarshalJSON(params)
	if err != nil {
		return types2.SwapClaims{}, err
	}

	route := fmt.Sprintf("custom/%s/%s", types2.ModuleName, types2.QueryGetSwapRewards)
	res, height, err := cliCtx.QueryWithData(route, bz)
	if err != nil {
		return types2.SwapClaims{}, err
	}
	...
	var claims types2.SwapClaims
	if err := cliCtx.LegacyAmino.UnmarshalJSON(res, &claims); err != nil {
		return types2.SwapClaims{}, fmt.Errorf("failed to unmarshal claims: %w", err)
	}

	return claims, nil
}

func executeSavingsRewardsQuery(cliCtx client.Context, params types2.QueryRewardsParams) (types2.SavingsClaims, error) {
	bz, err := cliCtx.LegacyAmino.MarshalJSON(params)
	if err != nil {
		return types2.SavingsClaims{}, err
	}

	route := fmt.Sprintf("custom/%s/%s", types2.ModuleName, types2.QueryGetSavingsRewards)
	res, height, err := cliCtx.QueryWithData(route, bz)
	if err != nil {
		return types2.SavingsClaims{}, err
	}
	...
	var claims types2.SavingsClaims
	if err := cliCtx.LegacyAmino.UnmarshalJSON(res, &claims); err != nil {
		return types2.SavingsClaims{}, fmt.Errorf("failed to unmarshal claims: %w", err)
	}
