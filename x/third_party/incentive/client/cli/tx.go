...
func getCmdClaimCdp() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`  $ %s tx %s claim-cdp large`, version.AppName, types2.ModuleName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types2.NewMsgClaimUSDXMintingReward(sender.String(), multiplier)
...
func getCmdClaimDelegator() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: strings.Join([]string{
			fmt.Sprintf(`  $ %s tx %s claim-delegator --%s jolt=large --%s swp=small`, version.AppName, types2.ModuleName, multiplierFlag, multiplierFlag),
			fmt.Sprintf(`  $ %s tx %s claim-delegator --%s jolt=large,swp=small`, version.AppName, types2.ModuleName, multiplierFlag),
		}, "\n"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types2.NewMsgClaimDelegatorReward(sender.String(), selections)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(cliCtx, cmd.Flags(), &msg)
...
func getCmdClaimSwap() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: strings.Join([]string{
			fmt.Sprintf(`  $ %s tx %s claim-swap --%s swp=large --%s ujolt=small`, version.AppName, types2.ModuleName, multiplierFlag, multiplierFlag),
			fmt.Sprintf(`  $ %s tx %s claim-swap --%s swp=large,ujolt=small`, version.AppName, types2.ModuleName, multiplierFlag),
		}, "\n"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types2.NewMsgClaimSwapReward(sender.String(), selections)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(cliCtx, cmd.Flags(), &msg)
...
func getCmdClaimSavings() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: strings.Join([]string{
			fmt.Sprintf(`  $ %s tx %s claim-savings --%s swp=large --%s ujolt=small`, version.AppName, types2.ModuleName, multiplierFlag, multiplierFlag),
			fmt.Sprintf(`  $ %s tx %s claim-savings --%s swp=large,ujolt=small`, version.AppName, types2.ModuleName, multiplierFlag),
		}, "\n"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types2.NewMsgClaimSavingsReward(sender.String(), selections)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(cliCtx, cmd.Flags(), &msg)
