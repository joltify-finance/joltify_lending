...
func (k Keeper) AccumulateJoltBorrowRewards(ctx sdk.Context, rewardPeriod types2.MultiRewardPeriod) {
	...
	indexes, found := k.GetJoltBorrowRewardIndexes(ctx, rewardPeriod.CollateralType)
	if !found {
	...
	totalSource := k.getJoltBorrowTotalSourceShares(ctx, rewardPeriod.CollateralType)

	acc.Accumulate(rewardPeriod, totalSource, ctx.BlockTime())

	k.SetPreviousJoltBorrowRewardAccrualTime(ctx, rewardPeriod.CollateralType, acc.PreviousAccumulationTime)
	if len(acc.Indexes) > 0 {
		// the store panics when setting empty or nil indexes
		k.SetJoltBorrowRewardIndexes(ctx, rewardPeriod.CollateralType, acc.Indexes)
	}
}

// getJoltBorrowTotalSourceShares fetches the sum of all source shares for a borrow reward.
//
// In the case of jolt borrow, this is the total borrowed divided by the borrow interest factor (for a particular denom).
...
func (k Keeper) InitializeJoltBorrowReward(ctx sdk.Context, borrow jolttypes.Borrow) {
	...
	var borrowRewardIndexes types2.MultiRewardIndexes
	for _, coin := range borrow.Amount {
		globalRewardIndexes, found := k.GetJoltBorrowRewardIndexes(ctx, coin.Denom)
		if !found {
...
func (k Keeper) SynchronizeJoltBorrowReward(ctx sdk.Context, borrow jolttypes.Borrow) {
	...
	// Source shares for jolt borrows is their normalized borrow amount
...
func (k Keeper) UpdateJoltBorrowIndexDenoms(ctx sdk.Context, borrow jolttypes.Borrow) {
	...
	for _, denom := range uniqueBorrowDenoms {
		globalBorrowRewardIndexes, found := k.GetJoltBorrowRewardIndexes(ctx, denom)
		if !found {
			globalBorrowRewardIndexes = types2.RewardIndexes{}
		}
...
// CalculateRewards computes how much rewards should have accrued to a reward source (eg a user's jolt borrowed btc amount)
