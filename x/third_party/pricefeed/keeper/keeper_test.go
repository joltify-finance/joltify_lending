...
func TestKeeper_ExpiredSetCurrentPrices(t *testing.T) {
	...
	err = keeper.SetCurrentPrices(ctx, "tstusd")
	require.ErrorIs(t, types2.ErrNoValidPrice, err, "there should be no valid prices to be set")

	_, err = keeper.GetCurrentPrice(ctx, "tstusd")
	require.ErrorIs(t, types2.ErrNoValidPrice, err, "current prices should be invalid")
}
