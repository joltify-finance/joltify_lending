...
func (suite *KeeperTestSuite) SetupTest() {
	...
	// this is the incentive module account
	modAccount, err := sdk.AccAddressFromBech32("jolt1hfny2zhlc328ksxjsv3qrrldcgqw3684ezcpu8")
...
func (suite *KeeperTestSuite) TestGetSetParams() {
	...
	suite.Require().Equal(types2.Params{Assets: []types2.Asset(nil)}, params)
	asset := types2.NewAsset(suite.addrs[0], "usdtoken", []string{suite.addrs[1]}, false, true, types2.NewRateLimit(false, sdk.ZeroInt(), time.Duration(0)))
	params = types2.NewParams([]types2.Asset{asset})
...
func (suite *KeeperTestSuite) TestIssueTokens() {
	...
	testCases := []struct {
		...
		},
		{
			...
			args{
				assets: []types2.Asset{
					types2.NewAsset(suite.addrs[0], "usdtoken", []string{suite.addrs[1]}, true, true, types2.NewRateLimit(false, sdk.ZeroInt(), time.Duration(0))),
				},
				sender:   suite.addrs[0],
				tokens:   sdk.NewCoin("usdtoken", sdk.NewInt(100000)),
				receiver: suite.addrs[1],
...
func (suite *KeeperTestSuite) TestRedeemTokens() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err := sk.MintCoins(suite.ctx, types2.ModuleAccountName, sdk.NewCoins(tc.args.initialTokens))
			suite.Require().NoError(err)
			sender, _ := sdk.AccAddressFromBech32(tc.args.sender)
			err = sk.SendCoinsFromModuleToAccount(suite.ctx, types2.ModuleAccountName, sender, sdk.NewCoins(tc.args.initialTokens))
			suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestBlockAddress() {
	...
	testCases := []struct {
		...
		},
		{
			...
			args{
				assets: []types2.Asset{
					types2.NewAsset(suite.addrs[0], "usdtoken", []string{}, false, false, types2.NewRateLimit(false, sdk.ZeroInt(), time.Duration(0))),
				},
				sender:      suite.addrs[0],
				blockedAddr: suite.addrs[1],
				denom:       "usdtoken",
...
func (suite *KeeperTestSuite) TestChangePauseStatus() {
	...
	testCases := []struct {
		...
		},
		{
			...
			args{
				assets: []types2.Asset{
					types2.NewAsset(suite.addrs[0], "usdtoken", []string{}, true, true, types2.NewRateLimit(false, sdk.ZeroInt(), time.Duration(0))),
				},
				sender:      suite.addrs[0],
				startStatus: true,
				endStatus:   false,
...
func (suite *KeeperTestSuite) TestSeizeCoinsFromBlockedAddress() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.SetupTest()
			assetsWithBlockedAddrs := []types2.Asset{}
			...
			params := types2.NewParams(assetsWithBlockedAddrs)
			suite.keeper.SetParams(suite.ctx, params)
			sk := suite.tApp.GetBankKeeper()
			for _, addrStr := range tc.args.blockedAddrs {
				addr, _ := sdk.AccAddressFromBech32(addrStr)
				err := sk.MintCoins(suite.ctx, types2.ModuleAccountName, sdk.NewCoins(tc.args.initialCoins))
				suite.Require().NoError(err)
				err = sk.SendCoinsFromModuleToAccount(suite.ctx, types2.ModuleAccountName, addr, sdk.NewCoins(tc.args.initialCoins))
			}
