...
func (k Keeper) CreateNewAssetSupply(ctx sdk.Context, denom string) types2.AssetSupply {
	supply := types2.NewAssetSupply(
...
func (k Keeper) IncrementCurrentAssetSupply(ctx sdk.Context, coin sdk.Coin) error {
	...
	if !found {
		return sdkerrors.Wrap(types2.ErrAssetNotFound, coin.Denom)
	}
	...
	if limit.Active {
		...
		if supplyLimit.IsLT(supply.CurrentSupply.Add(coin)) {
			return sdkerrors.Wrapf(types2.ErrExceedsSupplyLimit, "increase %s, asset supply %s, limit %s", coin, supply.CurrentSupply, supplyLimit)
		}
