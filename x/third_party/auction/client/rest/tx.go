...
func registerTxRoutes(cliCtx client.Context, r *mux.Router) {
	r.HandleFunc(fmt.Sprintf("/%s/auctions/{%s}/bids", types2.ModuleName, restAuctionID), postPlaceBidHandlerFn(cliCtx)).Methods("POST")
}
...
func postPlaceBidHandlerFn(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		msg := types2.NewMsgPlaceBid(auctionID, bidderAddr.String(), requestBody.Amount)
