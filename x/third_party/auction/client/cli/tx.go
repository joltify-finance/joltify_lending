...
func GetCmdPlaceBid() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf("  $ %s tx %s bid 34 1000usdx --from myKeyName", version.AppName, types2.ModuleName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types2.NewMsgPlaceBid(id, clientCtx.GetFromAddress().String(), amt)
