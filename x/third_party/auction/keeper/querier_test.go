...
func (suite *querierTestSuite) TestQueryParams() {
	bz, err := suite.querier(suite.Ctx, []string{types2.QueryGetParams}, abci.RequestQuery{})
	suite.Require().NoError(err)
...
func (suite *querierTestSuite) TestQueryAuction() {
	...
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, types2.QuerierRoute, types2.QueryGetAuction}, "/"),
		Data: suite.legacyAmino.MustMarshalJSON(types2.NewQueryAuctionParams(suite.auctions[0].GetID())),
	}

	// Execute query and check the []byte result
	bz, err := suite.querier(ctx, []string{types2.QueryGetAuction}, query)
	suite.NoError(err)
	suite.NotNil(bz)
...
func (suite *querierTestSuite) TestQueryAuctions() {
	...
	// Set up request query
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, types2.QuerierRoute, types2.QueryGetAuctions}, "/"),
		Data: suite.legacyAmino.MustMarshalJSON(
			types2.NewQueryAllAuctionParams(1, TestAuctionCount, "", "", "", nil),
		),
	}

	// Execute query and check the []byte result
	bz, err := suite.querier(ctx, []string{types2.QueryGetAuctions}, query)
	suite.Require().NoError(err)
	suite.Require().NotNil(bz)
...
func (suite *querierTestSuite) TestQueryNextAuctionID() {
	bz, err := suite.querier(suite.Ctx, []string{types2.QueryNextAuctionID}, abci.RequestQuery{})
	suite.Require().NoError(err)
	suite.Require().NotNil(bz)
