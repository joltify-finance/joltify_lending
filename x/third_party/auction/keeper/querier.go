...
func NewQuerier(keeper Keeper, legacyQuerierCdc *codec.LegacyAmino) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		switch path[0] {
		case types2.QueryGetAuction:
			return queryAuction(ctx, req, keeper, legacyQuerierCdc)
		case types2.QueryGetAuctions:
		...
		case types2.QueryNextAuctionID:
...
func queryAuction(ctx sdk.Context, req abci.RequestQuery, k Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	var params types2.QueryAuctionParams
	...
	if !ok {
		return nil, sdkerrors.Wrap(types2.ErrAuctionNotFound, fmt.Sprintf("%d", params.AuctionID))
	}
...
func queryAuctions(ctx sdk.Context, req abci.RequestQuery, k Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	var params types2.QueryAllAuctionParams
	err := legacyQuerierCdc.UnmarshalJSON(req.Data, &params)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONUnmarshal, err.Error())
	}
...
func filterAuctions(ctx sdk.Context, auctions []types2.Auction, params types2.QueryAllAuctionParams, legacyQuerierCdc *codec.LegacyAmino) []types2.Auction {
	filteredAuctions := make([]types2.Auction, 0, len(auctions))
	...
	if start < 0 || end < 0 {
		filteredAuctions = []types2.Auction{}
	} else {
...
func auctionIsMatch(auc types2.Auction, params types2.QueryAllAuctionParams) bool {
	...
	if len(params.Owner) > 0 {
		if cAuc, ok := auc.(*types2.CollateralAuction); ok {
