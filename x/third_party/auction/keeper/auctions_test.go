...
func (suite *auctionTestSuite) TestSurplusAuctionBasic() {
	...
	suite.Ctx = sdk.UnwrapSDKContext(suite.Ctx).WithBlockTime(sdk.UnwrapSDKContext(suite.Ctx).BlockTime().Add(types2.DefaultForwardBidDuration))
...
func (suite *auctionTestSuite) TestDebtAuctionBasic() {
	...
	ctx := sdk.UnwrapSDKContext(suite.Ctx).WithBlockTime(sdk.UnwrapSDKContext(suite.Ctx).BlockTime().Add(types2.DefaultForwardBidDuration))
...
func (suite *auctionTestSuite) TestCollateralAuctionBasic() {
	...
	// Close auction at just after auction expiry
	ctx := sdk.UnwrapSDKContext(suite.Ctx).WithBlockTime(sdk.UnwrapSDKContext(suite.Ctx).BlockTime().Add(types2.DefaultReverseBidDuration))
	suite.NoError(suite.Keeper.CloseAuction(ctx, auctionID))
	// Check buyer's coins increased
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.expectPass {
				...
				surplusAuction := types2.SurplusAuction{BaseAuction: types2.BaseAuction{
					ID:              id,
					...
					EndTime:         types2.DistantFuture,
					MaxEndTime:      types2.DistantFuture,
				}}
...
func (suite *auctionTestSuite) TestCloseExpiredAuctions() {
	...
	ctx := sdk.UnwrapSDKContext(suite.Ctx).WithBlockTime(sdk.UnwrapSDKContext(suite.Ctx).BlockTime().Add(types2.DefaultMaxAuctionDuration).Add(1))
