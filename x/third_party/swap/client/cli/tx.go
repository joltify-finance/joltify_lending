...
func getCmdDeposit() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s deposit 10000000ujolt 10000000usdx 0.01 1624224736 --from <key>`,
...
func getCmdWithdraw() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s withdraw 153000 10000000ujolt 20000000usdx 176293740 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func getCmdSwapExactForTokens() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s swap-exact-for-tokens 1000000ujolt 5000000usdc 0.01 1624224736 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fromAddr := clientCtx.GetFromAddress()
			msg := types.NewMsgSwapExactForBatchTokens(fromAddr.String(), exactTokenA, tokenB, args[2], deadline)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
...
func getCmdSwapExactForBatchTokens() *cobra.Command {
	return &cobra.Command{
		Use:   "swap-exact-for-Batch tokens [exactCoinA] [coinB] [slippage] [deadline]",
		Short: "swap an exact amount of token a for token b",
		Example: fmt.Sprintf(
			`%s tx %s swap-exact-for-tokens 1000000usdt 5000000usdc 0.01 1624224736 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func getCmdSwapForExactTokens() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s swap-for-exact-tokens 1000000ujolt 5000000usdx 0.01 1624224736 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
