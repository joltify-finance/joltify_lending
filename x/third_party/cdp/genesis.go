...
func InitGenesis(ctx sdk.Context, k keeper.Keeper, pk types2.PricefeedKeeper, ak types2.AccountKeeper, gs types2.GenesisState) {
	if err := gs.Validate(); err != nil {
		panic(fmt.Sprintf("failed to validate %s genesis state: %s", types2.ModuleName, err))
	}

	// check if the module accounts exists
	cdpModuleAcc := ak.GetModuleAccount(ctx, types2.ModuleName)
	if cdpModuleAcc == nil {
		panic(fmt.Sprintf("%s module account has not been set", types2.ModuleName))
	}
	liqModuleAcc := ak.GetModuleAccount(ctx, types2.LiquidatorMacc)
	if liqModuleAcc == nil {
		panic(fmt.Sprintf("%s module account has not been set", types2.LiquidatorMacc))
	}
	...
	for _, gtp := range gs.TotalPrincipals {
		k.SetTotalPrincipal(ctx, gtp.CollateralType, types2.DefaultStableDenom, gtp.TotalPrincipal)
	}
	// add cdps
...
func ExportGenesis(ctx sdk.Context, k keeper.Keeper) types2.GenesisState {
	...
	k.IterateAllCdps(ctx, func(cdp types2.CDP) (stop bool) {
		syncedCdp := k.SynchronizeInterest(ctx, cdp)
		cdps = append(cdps, syncedCdp)
		k.IterateDeposits(ctx, cdp.ID, func(deposit types2.Deposit) (stop bool) {
	...
	var previousAccumTimes types2.GenesisAccumulationTimes
	...
	for _, cp := range params.CollateralParams {
		...
		previousAccumTimes = append(previousAccumTimes, types2.NewGenesisAccumulationTime(cp.Type, previousAccumTime, interestFactor))

		tp := k.GetTotalPrincipal(ctx, cp.Type, types2.DefaultStableDenom)
		genTotalPrincipal := types2.NewGenesisTotalPrincipal(cp.Type, tp)
		totalPrincipals = append(totalPrincipals, genTotalPrincipal)
	}

	return types2.NewGenesisState(params, cdps, deposits, cdpID, debtDenom, govDenom, previousAccumTimes, totalPrincipals)
}
