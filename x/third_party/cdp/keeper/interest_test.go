...
func (suite *InterestTestSuite) TestCalculateInterestFactor() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			interestFactor := keeper2.CalculateInterestFactor(tc.args.perSecondInterestRate, tc.args.timeElapsed)
...
func (suite *InterestTestSuite) TestAccumulateInterest() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.keeper.SetTotalPrincipal(suite.ctx, tc.args.ctype, types2.DefaultStableDenom, tc.args.totalPrincipal)
			...
			actualTotalPrincipal := suite.keeper.GetTotalPrincipal(suite.ctx, tc.args.ctype, types2.DefaultStableDenom)
...
func (suite *InterestTestSuite) TestSynchronizeInterest() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err := bk.MintCoins(suite.ctx, types2.ModuleName, cs(tc.args.initialCollateral))
			suite.Require().NoError(err)
			err = bk.SendCoinsFromModuleToAccount(suite.ctx, types2.ModuleName, addrs[0], cs(tc.args.initialCollateral))
			suite.Require().NoError(err)
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			for j := 0; j < tc.args.numberOfCdps; j++ {
				...
				ak.SetAccount(suite.ctx, acc)
				bk := suite.app.GetBankKeeper()
				err := bk.MintCoins(suite.ctx, types2.ModuleName, cs(tc.args.initialCDPCollateral))
				suite.Require().NoError(err)
				err = bk.SendCoinsFromModuleToAccount(suite.ctx, types2.ModuleName, addrs[j], cs(tc.args.initialCDPCollateral))
				suite.Require().NoError(err)
			...
			bk := suite.app.GetBankKeeper()
			debtSupply := bk.GetSupply(suite.ctx, types2.DefaultDebtDenom)
			usdxSupply := bk.GetSupply(suite.ctx, types2.DefaultStableDenom)
			totalPrincipal := suite.keeper.GetTotalPrincipal(suite.ctx, tc.args.ctype, types2.DefaultStableDenom)
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			for _, addr := range addrs {
				...
				err := bk.MintCoins(suite.ctx, types2.ModuleName, cs(tc.args.initialCollateral))
				suite.Require().NoError(err)
				err = bk.SendCoinsFromModuleToAccount(suite.ctx, types2.ModuleName, addr, cs(tc.args.initialCollateral))
				suite.Require().NoError(err)
			}
			// setup pricefeed
			pk := suite.app.GetPriceFeedKeeper()
