...
func (suite *SeizeTestSuite) TestSeizeCollateral() {
	...
	_, found = auctionKeeper.GetAuction(suite.ctx, types3.DefaultNextAuctionID)
	suite.True(found)

	auctionMacc := ak.GetModuleAccount(suite.ctx, types3.ModuleName)
...
func (suite *SeizeTestSuite) TestKeeperLiquidation() {
	...
	addr, _ := sdk.AccAddressFromBech32("jolt1ze7y9qwdddejmy7jlw4cymqqlt2wh05ypmjzfy")
			...
			args{
				...
				expectedAuctions: []types3.Auction{
					&types3.CollateralAuction{
						...
						LotReturns: types3.WeightedAddresses{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.errArgs.expectLiquidate {
				...
				//for i := range tc.args.expectedAuctions {
				//	a := tc.args.expectedAuctions[i]
				//	b := auctions[i]
				//	suite.Require().Equal(b.String(), a.String())
				//}
