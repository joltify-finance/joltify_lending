...
func NewQueryServerImpl(keeper Keeper) types2.QueryServer {
...
func (s QueryServer) Accounts(c context.Context, req *types2.QueryAccountsRequest) (*types2.QueryAccountsResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)

	cdpAccAccount := s.keeper.accountKeeper.GetModuleAccount(ctx, types2.ModuleName)
	liquidatorAccAccount := s.keeper.accountKeeper.GetModuleAccount(ctx, types2.LiquidatorMacc)
	...
	return &types2.QueryAccountsResponse{Accounts: accounts}, nil
}

// TotalPrincipal queries the total principal of a given collateral type.
func (s QueryServer) TotalPrincipal(c context.Context, req *types2.QueryTotalPrincipalRequest) (*types2.QueryTotalPrincipalResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	...
	var collateralPrincipals types2.TotalPrincipals

	for _, queryType := range queryCollateralTypes {
		// Hardcoded to default USDX
		principalAmount := s.keeper.GetTotalPrincipal(ctx, queryType, types2.DefaultStableDenom)
		// Wrap it in an sdk.Coin
		totalAmountCoin := sdk.NewCoin(types2.DefaultStableDenom, principalAmount)

		totalPrincipal := types2.NewTotalPrincipal(queryType, totalAmountCoin)
		collateralPrincipals = append(collateralPrincipals, totalPrincipal)
	}

	return &types2.QueryTotalPrincipalResponse{
		TotalPrincipal: collateralPrincipals,
	}, nil
}

// TotalCollateral queries the total collateral of a given collateral type.
func (s QueryServer) TotalCollateral(c context.Context, req *types2.QueryTotalCollateralRequest) (*types2.QueryTotalCollateralResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	...
	cdpAccount := s.keeper.accountKeeper.GetModuleAccount(ctx, types2.ModuleName)
	totalCdpCollateral := s.keeper.bankKeeper.GetAllBalances(ctx, cdpAccount.GetAddress())

	var totalCollaterals types2.TotalCollaterals
	...
	for denom, collateralTypes := range denomCollateralTypes {
		...
		for i := len(collateralTypes) - 1; i > 0; i-- {
			...
			if req.CollateralType == "" || collateralTypes[i] == req.CollateralType {
				totalCollaterals = append(totalCollaterals, types2.NewTotalCollateral(collateralTypes[i], sdk.NewCoin(denom, collateral)))
			}
		...
		if req.CollateralType == "" || collateralTypes[0] == req.CollateralType {
			...
			totalCollaterals = append(totalCollaterals, types2.NewTotalCollateral(collateralTypes[0], sdk.NewCoin(denom, totalCollateral)))
		}
	}
	...
	return &types2.QueryTotalCollateralResponse{
		TotalCollateral: totalCollaterals,
	}, nil
}

// Cdps queries all active CDPs.
func (s QueryServer) Cdps(c context.Context, req *types2.QueryCdpsRequest) (*types2.QueryCdpsResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	...
	return &types2.QueryCdpsResponse{
		Cdps: filteredCDPs,
...
func (s QueryServer) Cdp(c context.Context, req *types2.QueryCdpRequest) (*types2.QueryCdpResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	...
	if !valid {
		return nil, sdkerrors.Wrap(types2.ErrInvalidCollateral, req.CollateralType)
	}

	cdp, found := s.keeper.GetCdpByOwnerAndCollateralType(ctx, owner, req.CollateralType)
	if !found {
		return nil, sdkerrors.Wrapf(types2.ErrCdpNotFound, "owner %s, denom %s", req.Owner, req.CollateralType)
	}

	cdpResponse := s.keeper.LoadCDPResponse(ctx, cdp)

	return &types2.QueryCdpResponse{
		Cdp: cdpResponse,
	}, nil
}

// Deposits queries deposits associated with the CDP owned by an address for a collateral type.
func (s QueryServer) Deposits(c context.Context, req *types2.QueryDepositsRequest) (*types2.QueryDepositsResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
...
func GrpcFilterCDPs(ctx sdk.Context, k Keeper, req types2.QueryCdpsRequest) (types2.CDPResponses, error) {
	...
	legacyParams := types2.NewQueryCdpsParams(page, limit, req.CollateralType, owner, req.ID, ratio)
	...
	var cdpResponses types2.CDPResponses
	for _, cdp := range cdps {
		cdpResponse := types2.CDPResponse{
			ID:                     cdp.ID,
