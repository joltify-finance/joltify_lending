...
func (suite *CdpTestSuite) TestAddCdp() {
	...
	suite.Require().True(errors.Is(err, types2.ErrInvalidCollateral))
	...
	suite.Require().True(errors.Is(err, types2.ErrDebtNotSupported))
	...
	suite.Require().True(errors.Is(err, types2.ErrExceedsDebtLimit))
	...
	suite.Require().True(errors.Is(err, types2.ErrPricefeedDown))
	...
	macc := ak.GetModuleAccount(suite.ctx, types2.ModuleName)
	...
	suite.Require().True(errors.Is(err, types2.ErrCollateralNotSupported))
	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 100), c("usdx", 10), "xrp-a")
	suite.Require().True(errors.Is(err, types2.ErrCdpAlreadyExists))
}
...
func (suite *CdpTestSuite) TestGetNextCdpID() {
	id := suite.keeper.GetNextCdpID(suite.ctx)
	suite.Equal(types2.DefaultCdpStartingID, id)
}

func (suite *CdpTestSuite) TestGetSetCdp() {
	_, addrs := app.GeneratePrivKeyAddressPairs(1)
	cdp := types2.NewCDP(types2.DefaultCdpStartingID, addrs[0], c("xrp", 1), "xrp-a", c("usdx", 1), tmtime.Canonical(time.Now()), sdk.OneDec())
	err := suite.keeper.SetCDP(suite.ctx, cdp)
	suite.NoError(err)

	t, found := suite.keeper.GetCDP(suite.ctx, "xrp-a", types2.DefaultCdpStartingID)
	suite.True(found)
	suite.Equal(cdp, t)
	_, found = suite.keeper.GetCDP(suite.ctx, "xrp-a", uint64(2))
	suite.False(found)
	suite.NoError(suite.keeper.DeleteCDP(suite.ctx, cdp))
	_, found = suite.keeper.GetCDP(suite.ctx, "btc-a", types2.DefaultCdpStartingID)
	suite.False(found)
}
...
func (suite *CdpTestSuite) TestCalculateCollateralToDebtRatio() {
	_, addrs := app.GeneratePrivKeyAddressPairs(1)
	cdp := types2.NewCDP(types2.DefaultCdpStartingID, addrs[0], c("xrp", 3), "xrp-a", c("usdx", 1), tmtime.Canonical(time.Now()), sdk.OneDec())
	...
	cdp = types2.NewCDP(types2.DefaultCdpStartingID, addrs[0], c("xrp", 1), "xrp-a", c("usdx", 2), tmtime.Canonical(time.Now()), sdk.OneDec())
	cr = suite.keeper.CalculateCollateralToDebtRatio(suite.ctx, cdp.Collateral, cdp.Type, cdp.Principal)
...
func (suite *CdpTestSuite) TestMintBurnDebtCoins() {
	cd := cdps()[1]
	err := suite.keeper.MintDebtCoins(suite.ctx, types2.ModuleName, suite.keeper.GetDebtDenom(suite.ctx), cd.Principal)
	suite.NoError(err)
	...
	err = suite.keeper.BurnDebtCoins(suite.ctx, types2.ModuleName, suite.keeper.GetDebtDenom(suite.ctx), cd.Principal)
	suite.NoError(err)
	suite.Require().Panics(func() {
...
func (suite *CdpTestSuite) TestMarketStatus() {
	suite.keeper.SetMarketStatus(suite.ctx, "ujolt:usd", true)
	status := suite.keeper.GetMarketStatus(suite.ctx, "ujolt:usd")
	suite.Require().True(status)
	suite.keeper.SetMarketStatus(suite.ctx, "ujolt:usd", false)
	status = suite.keeper.GetMarketStatus(suite.ctx, "ujolt:usd")
