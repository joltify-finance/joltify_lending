...
func (suite *DepositTestSuite) TestGetSetDeposit() {
	...
	suite.True(found)
	td := types2.NewDeposit(uint64(1), suite.addrs[0], c("xrp", 400000000))
...
func (suite *DepositTestSuite) TestDepositCollateral() {
	...
	d, found := suite.keeper.GetDeposit(suite.ctx, uint64(1), suite.addrs[0])
	suite.True(found)
	td := types2.NewDeposit(uint64(1), suite.addrs[0], c("xrp", 410000000))
	suite.True(d.Equals(td))
	ds := suite.keeper.GetDeposits(suite.ctx, uint64(1))
	suite.Equal(1, len(ds))
	suite.True(ds[0].Equals(td))
	...
	suite.True(found)
	td = types2.NewDeposit(uint64(1), suite.addrs[1], c("xrp", 10000000))
	suite.True(d.Equals(td))
	ds = suite.keeper.GetDeposits(suite.ctx, uint64(1))
...
func (suite *DepositTestSuite) TestWithdrawCollateral() {
	...
	td := types2.NewDeposit(uint64(1), suite.addrs[0], c("xrp", 390000000))
	...
	suite.Require().True(errors.Is(err, types2.ErrDepositNotFound))
}
