...
func (k Keeper) AddPrincipal(ctx sdk.Context, owner sdk.AccAddress, collateralType string, principal sdk.Coin) error {
	...
	if !found {
		return sdkerrors.Wrapf(types2.ErrCdpNotFound, "owner %s, denom %s", owner, collateralType)
	}
...
func (k Keeper) RepayPrincipal(ctx sdk.Context, owner sdk.AccAddress, collateralType string, payment sdk.Coin) error {
	...
	err = k.bankKeeper.SendCoinsFromAccountToModule(ctx, owner, types2.ModuleName, sdk.NewCoins(feePayment.Add(principalPayment)))
	if err != nil {
		return err
	}

	// burn the payment coins
	err = k.bankKeeper.BurnCoins(ctx, types2.ModuleName, sdk.NewCoins(feePayment.Add(principalPayment)))
	if err != nil {
		panic(err)
	}

	// burn the corresponding amount of debt coins
	cdpDebt := k.getModAccountDebt(ctx, types2.ModuleName)
	...
	err = k.BurnDebtCoins(ctx, types2.ModuleName, debtDenom, coinsToBurn)
...
func (k Keeper) ValidatePaymentCoins(ctx sdk.Context, cdp types2.CDP, payment sdk.Coin) error {
	debt := cdp.GetTotalPrincipal()
	if payment.Denom != debt.Denom {
		return sdkerrors.Wrapf(types2.ErrInvalidPayment, "cdp %d: expected %s, got %s", cdp.ID, debt.Denom, payment.Denom)
	}
	_, found := k.GetDebtParam(ctx, payment.Denom)
	if !found {
		return sdkerrors.Wrapf(types2.ErrInvalidPayment, "payment denom %s not found", payment.Denom)
	}
	return nil
}

// ReturnCollateral returns collateral to depositors on a cdp and removes deposits from the store
func (k Keeper) ReturnCollateral(ctx sdk.Context, cdp types2.CDP) {
	deposits := k.GetDeposits(ctx, cdp.ID)
	for _, deposit := range deposits {
		if err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types2.ModuleName, deposit.Depositor, sdk.NewCoins(deposit.Amount)); err != nil {
			panic(err)
		}
...
func (k Keeper) validatePrincipalPayment(ctx sdk.Context, cdp types2.CDP, payment sdk.Coin) error {
	...
	if proposedBalance.GT(sdk.ZeroInt()) && proposedBalance.LT(dp.DebtFloor) {
		return sdkerrors.Wrapf(types2.ErrBelowDebtFloor, "proposed %s < minimum %s", sdk.NewCoin(payment.Denom, proposedBalance), dp.DebtFloor)
	}
	return nil
}
