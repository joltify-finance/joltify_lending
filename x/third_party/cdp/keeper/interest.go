...
func (k Keeper) AccumulateInterest(ctx sdk.Context, ctype string) error {
	...
	totalPrincipalPrior := k.GetTotalPrincipal(ctx, ctype, types2.DefaultStableDenom)
	...
	err := k.MintDebtCoins(ctx, types2.ModuleName, k.GetDebtDenom(ctx), sdk.NewCoin(types2.DefaultStableDenom, interestAccumulated))
	if err != nil {
		return err
	}

	dp, found := k.GetDebtParam(ctx, types2.DefaultStableDenom)
	if !found {
		panic(fmt.Sprintf("Debt parameters for %s not found", types2.DefaultStableDenom))
	}
	...
	if newFeesSurplus.IsPositive() {
		err := k.bankKeeper.MintCoins(ctx, types2.LiquidatorMacc, sdk.NewCoins(sdk.NewCoin(dp.Denom, newFeesSurplus)))
		if err != nil {
			return err
		}
	}
	...
	k.SetTotalPrincipal(ctx, ctype, types2.DefaultStableDenom, totalPrincipalNew)
...
func (k Keeper) SynchronizeInterest(ctx sdk.Context, cdp types2.CDP) types2.CDP {
...
func (k Keeper) CalculateNewInterest(ctx sdk.Context, cdp types2.CDP) sdk.Coin {
	globalInterestFactor, found := k.GetInterestFactor(ctx, cdp.Type)
	if !found {
