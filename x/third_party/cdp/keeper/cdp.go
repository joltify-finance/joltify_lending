...
func (k Keeper) AddCdp(ctx sdk.Context, owner sdk.AccAddress, collateral sdk.Coin, principal sdk.Coin, collateralType string) error {
	...
	if found {
		return sdkerrors.Wrapf(types2.ErrCdpAlreadyExists, "owner %s, denom %s", owner, collateral.Denom)
	}
	...
	cdp := types2.NewCDP(id, owner, collateral, collateralType, principal, ctx.BlockHeader().Time, interestFactor)
	deposit := types2.NewDeposit(cdp.ID, owner, collateral)
	err = k.bankKeeper.SendCoinsFromAccountToModule(ctx, owner, types2.ModuleName, sdk.NewCoins(collateral))
	if err != nil {
		return err
	}

	// mint the principal and send to the owners account
	err = k.bankKeeper.MintCoins(ctx, types2.ModuleName, sdk.NewCoins(principal))
	if err != nil {
		panic(err)
	}
	err = k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types2.ModuleName, owner, sdk.NewCoins(principal))
	if err != nil {
		panic(err)
	}

	// mint the corresponding amount of debt coins
	err = k.MintDebtCoins(ctx, types2.ModuleName, k.GetDebtDenom(ctx), principal)
	if err != nil {
		panic(err)
	}
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			types2.EventTypeCreateCdp,
			sdk.NewAttribute(types2.AttributeKeyCdpID, fmt.Sprintf("%d", cdp.ID)),
		),
	)
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			types2.EventTypeCdpDeposit,
...
func (k Keeper) UpdateCdpAndCollateralRatioIndex(ctx sdk.Context, cdp types2.CDP, ratio sdk.Dec) error {
...
func (k Keeper) DeleteCdpAndCollateralRatioIndex(ctx sdk.Context, cdp types2.CDP) error {
	err := k.removeOldCollateralRatioIndex(ctx, cdp.Type, cdp.ID)
	if err != nil {
		return err
	}
...
func (k Keeper) SetCdpAndCollateralRatioIndex(ctx sdk.Context, cdp types2.CDP, ratio sdk.Dec) error {
	err := k.SetCDP(ctx, cdp)
	if err != nil {
		return err
	}
...
func (k Keeper) removeOldCollateralRatioIndex(ctx sdk.Context, ctype string, id uint64) error {
	...
	if !found {
		return sdkerrors.Wrapf(types2.ErrCdpNotFound, "%d", storedCDP.ID)
	}
...
func (k Keeper) GetCdpIdsByOwner(ctx sdk.Context, owner sdk.AccAddress) ([]uint64, bool) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpIDKeyPrefix)
	bz := store.Get(owner)
	if bz == nil {
		return []uint64{}, false
	}

	var index types2.OwnerCDPIndex
...
func (k Keeper) GetCdpByOwnerAndCollateralType(ctx sdk.Context, owner sdk.AccAddress, collateralType string) (types2.CDP, bool) {
	cdpIDs, found := k.GetCdpIdsByOwner(ctx, owner)
	if !found {
		return types2.CDP{}, false
	}
	for _, id := range cdpIDs {
...
func (k Keeper) GetCDP(ctx sdk.Context, collateralType string, cdpID uint64) (types2.CDP, bool) {
	// get store
	...
	// get CDP
	bz := store.Get(types2.CdpKey(collateralType, cdpID))
	// unmarshal
	if bz == nil {
		return types2.CDP{}, false
	}
...
func (k Keeper) SetCDP(ctx sdk.Context, cdp types2.CDP) error {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpKeyPrefix)
	_, found := k.GetCollateral(ctx, cdp.Type)
	if !found {
		return sdkerrors.Wrapf(types2.ErrDenomPrefixNotFound, "%s", cdp.Collateral.Denom)
	}
	bz := k.cdc.MustMarshal(&cdp)
	store.Set(types2.CdpKey(cdp.Type, cdp.ID), bz)
	return nil
}

// DeleteCDP deletes a cdp from the store
func (k Keeper) DeleteCDP(ctx sdk.Context, cdp types2.CDP) error {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpKeyPrefix)
	_, found := k.GetCollateral(ctx, cdp.Type)
	if !found {
		return sdkerrors.Wrapf(types2.ErrDenomPrefixNotFound, "%s", cdp.Collateral.Denom)
	}
	store.Delete(types2.CdpKey(cdp.Type, cdp.ID))
	return nil
}

// GetAllCdps returns all cdps from the store
func (k Keeper) GetAllCdps(ctx sdk.Context) (cdps types2.CDPs) {
	k.IterateAllCdps(ctx, func(cdp types2.CDP) bool {
...
func (k Keeper) GetAllCdpsByCollateralType(ctx sdk.Context, collateralType string) (cdps types2.CDPs) {
	k.IterateCdpsByCollateralType(ctx, collateralType, func(cdp types2.CDP) bool {
		cdps = append(cdps, cdp)
		return false
	})
	return
...
func (k Keeper) GetAllCdpsByCollateralTypeAndRatio(ctx sdk.Context, collateralType string, targetRatio sdk.Dec) (cdps types2.CDPs) {
...
func (k Keeper) SetNextCdpID(ctx sdk.Context, id uint64) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpIDKey)
	store.Set(types2.CdpIDKey, types2.GetCdpIDBytes(id))
}

// GetNextCdpID returns the highest cdp id from the store
func (k Keeper) GetNextCdpID(ctx sdk.Context) (id uint64) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpIDKey)
	bz := store.Get(types2.CdpIDKey)
	if bz == nil {
		panic("starting cdp id not set in genesis")
	}
	id = types2.GetCdpIDFromBytes(bz)
	return
}

// IndexCdpByOwner sets the cdp id in the store, indexed by the owner
func (k Keeper) IndexCdpByOwner(ctx sdk.Context, cdp types2.CDP) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpIDKeyPrefix)
	...
	newIndex := types2.OwnerCDPIndex{CdpIDs: cdpIDs}
...
func (k Keeper) RemoveCdpOwnerIndex(ctx sdk.Context, cdp types2.CDP) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.CdpIDKeyPrefix)
	...
	updatedIndex := types2.OwnerCDPIndex{CdpIDs: updatedCdpIds}
...
func (k Keeper) IndexCdpByCollateralRatio(ctx sdk.Context, collateralType string, id uint64, collateralRatio sdk.Dec) {
	...
	store.Set(types2.CollateralRatioKey(collateralType, id, collateralRatio), types2.GetCdpIDBytes(id))
}
...
func (k Keeper) RemoveCdpCollateralRatioIndex(ctx sdk.Context, collateralType string, id uint64, collateralRatio sdk.Dec) {
	...
	store.Delete(types2.CollateralRatioKey(collateralType, id, collateralRatio))
}

// GetDebtDenom returns the denom of debt in the system
func (k Keeper) GetDebtDenom(ctx sdk.Context) string {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.DebtDenomKey)
	bz := store.Get(types2.DebtDenomKey)
	return string(bz)
}

// GetGovDenom returns the denom of the governance token
func (k Keeper) GetGovDenom(ctx sdk.Context) string {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.GovDenomKey)
	bz := store.Get(types2.GovDenomKey)
	return string(bz)
}
...
func (k Keeper) SetDebtDenom(ctx sdk.Context, denom string) {
	...
	store := prefix.NewStore(ctx.KVStore(k.key), types2.DebtDenomKey)
	store.Set(types2.DebtDenomKey, []byte(denom))
}
...
func (k Keeper) SetGovDenom(ctx sdk.Context, denom string) {
	...
	store := prefix.NewStore(ctx.KVStore(k.key), types2.GovDenomKey)
	store.Set(types2.GovDenomKey, []byte(denom))
}
...
func (k Keeper) ValidateCollateral(ctx sdk.Context, collateral sdk.Coin, collateralType string) error {
	...
	if !found {
		return sdkerrors.Wrap(types2.ErrCollateralNotSupported, collateral.Denom)
	}
	if cp.Denom != collateral.Denom {
		return sdkerrors.Wrapf(types2.ErrInvalidCollateral, "collateral type: %s expected denom: %s got: %s", collateralType, cp.Denom, collateral.Denom)
	}
	ok := k.GetMarketStatus(ctx, cp.SpotMarketID)
	if !ok {
		return sdkerrors.Wrap(types2.ErrPricefeedDown, collateral.Denom)
	}
...
func (k Keeper) ValidatePrincipalAdd(ctx sdk.Context, principal sdk.Coin) error {
	...
	if !found {
		return sdkerrors.Wrap(types2.ErrDebtNotSupported, principal.Denom)
	}
	if principal.Amount.LT(dp.DebtFloor) {
		return sdkerrors.Wrapf(types2.ErrBelowDebtFloor, "proposed %s < minimum %s", principal, dp.DebtFloor)
	}
	return nil
}
...
func (k Keeper) ValidatePrincipalDraw(ctx sdk.Context, principal sdk.Coin, expectedDenom string) error {
	if principal.Denom != expectedDenom {
		return sdkerrors.Wrapf(types2.ErrInvalidDebtRequest, "proposed %s, expected %s", principal.Denom, expectedDenom)
	}
...
func (k Keeper) ValidateDebtLimit(ctx sdk.Context, collateralType string, principal sdk.Coin) error {
	cp, found := k.GetCollateral(ctx, collateralType)
	if !found {
		return sdkerrors.Wrap(types2.ErrCollateralNotSupported, collateralType)
	}
	...
	if totalPrincipal.GT(collateralLimit) {
		return sdkerrors.Wrapf(types2.ErrExceedsDebtLimit, "debt increase %s > collateral debt limit %s", sdk.NewCoins(sdk.NewCoin(principal.Denom, totalPrincipal)), sdk.NewCoins(sdk.NewCoin(principal.Denom, collateralLimit)))
	}
	globalLimit := k.GetParams(ctx).GlobalDebtLimit.Amount
	if totalPrincipal.GT(globalLimit) {
		return sdkerrors.Wrapf(types2.ErrExceedsDebtLimit, "debt increase %s > global debt limit  %s", sdk.NewCoin(principal.Denom, totalPrincipal), sdk.NewCoin(principal.Denom, globalLimit))
	}
	return nil
}
...
func (k Keeper) ValidateCollateralizationRatio(ctx sdk.Context, collateral sdk.Coin, collateralType string, principal sdk.Coin, fees sdk.Coin) error {
	...
	if collateralizationRatio.LT(liquidationRatio) {
		return sdkerrors.Wrapf(types2.ErrInvalidCollateralRatio, "collateral %s, collateral ratio %s, liquidation ratio %s", collateral.Denom, collateralizationRatio, liquidationRatio)
	}
	return nil
}
...
func (k Keeper) ValidateBalance(ctx sdk.Context, amount sdk.Coin, sender sdk.AccAddress) error {
	...
	if acc == nil {
		return sdkerrors.Wrapf(types2.ErrAccountNotFound, "address: %s", sender)
	}
	...
	if spendableBalance.LT(amount.Amount) {
		return sdkerrors.Wrapf(types2.ErrInsufficientBalance, "%s < %s", sdk.NewCoin(amount.Denom, spendableBalance), amount)
	}
...
func (k Keeper) CalculateCollateralToDebtRatio(ctx sdk.Context, collateral sdk.Coin, collateralType string, debt sdk.Coin) sdk.Dec {
	...
	if debtTotal.IsZero() || debtTotal.GTE(types2.MaxSortableDec) {
		return types2.MaxSortableDec.Sub(sdk.SmallestDec())
	}
...
func (k Keeper) LoadAugmentedCDP(ctx sdk.Context, cdp types2.CDP) types2.AugmentedCDP {
	...
	if err != nil {
		return types2.AugmentedCDP{CDP: cdp}
	}
	...
	augmentedCDP := types2.NewAugmentedCDP(cdp, collateralValueInDebt, collateralizationRatio)
	return augmentedCDP
}

// LoadCDPResponse creates a new CDPResponse from an existing CDP
func (k Keeper) LoadCDPResponse(ctx sdk.Context, cdp types2.CDP) types2.CDPResponse {
	// sync the latest interest of the cdp
	interestAccumulated := k.CalculateNewInterest(ctx, cdp)
	cdp.AccumulatedFees = cdp.AccumulatedFees.Add(interestAccumulated)
	// update cdp fields to match synced accumulated fees
	...
	return types2.NewCDPResponse(cdp, collateralValueInDebt, collateralizationRatio)
}
...
func (k Keeper) SetMarketStatus(ctx sdk.Context, marketID string, up bool) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.PricefeedStatusKeyPrefix)
	if up {
