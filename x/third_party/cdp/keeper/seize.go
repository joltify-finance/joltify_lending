...
func (k Keeper) SeizeCollateral(ctx sdk.Context, cdp types2.CDP) error {
	...
	modAccountDebt := k.getModAccountDebt(ctx, types2.ModuleName)
	debt = sdk.MinInt(debt, modAccountDebt)
	debtCoin := sdk.NewCoin(k.GetDebtDenom(ctx), debt)
	err := k.bankKeeper.SendCoinsFromModuleToModule(ctx, types2.ModuleName, types2.LiquidatorMacc, sdk.NewCoins(debtCoin))
	if err != nil {
		return err
	}
	...
	for _, dep := range deposits {
		if err := k.bankKeeper.SendCoinsFromModuleToModule(ctx, types2.ModuleName, types2.LiquidatorMacc, sdk.NewCoins(dep.Amount)); err != nil {
			return err
		}
		...
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types2.EventTypeCdpLiquidation,
				...
				sdk.NewAttribute(types2.AttributeKeyDeposit, dep.String()),
			),
		)
	}
...
func (k Keeper) ValidateLiquidation(ctx sdk.Context, collateral sdk.Coin, collateralType string, principal sdk.Coin, fees sdk.Coin) error {
	...
	if collateralizationRatio.GT(liquidationRatio) {
		return sdkerrors.Wrapf(types2.ErrNotLiquidatable, "collateral %s, collateral ratio %s, liquidation ratio %s", collateral.Denom, collateralizationRatio, liquidationRatio)
	}
	return nil
}
...
func (k Keeper) payoutKeeperLiquidationReward(ctx sdk.Context, keeper sdk.AccAddress, cdp types2.CDP) (types2.CDP, error) {
	collateralParam, found := k.GetCollateral(ctx, cdp.Type)
	if !found {
		return types2.CDP{}, sdkerrors.Wrapf(types2.ErrInvalidCollateral, "%s", cdp.Type)
	}
	collateralDec := sdk.NewDecFromInt(cdp.Collateral.Amount)
	reward := collateralDec.Mul(collateralParam.KeeperRewardPercentage).RoundInt()
	...
	err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types2.ModuleName, keeper, sdk.NewCoins(rewardCoin))
	if err != nil {
		return types2.CDP{}, err
	}
