...
func (k Keeper) DepositCollateral(ctx sdk.Context, owner, depositor sdk.AccAddress, collateral sdk.Coin, collateralType string) error {
	...
	if !found {
		return sdkerrors.Wrapf(types2.ErrCdpNotFound, "owner %s, collateral %s", owner, collateralType)
	}
	...
	if found {
		...
		deposit = types2.NewDeposit(cdp.ID, depositor, collateral)
	}
	err = k.bankKeeper.SendCoinsFromAccountToModule(ctx, depositor, types2.ModuleName, sdk.NewCoins(collateral))
	if err != nil {
		return err
	}
...
func (k Keeper) WithdrawCollateral(ctx sdk.Context, owner, depositor sdk.AccAddress, collateral sdk.Coin, collateralType string) error {
	...
	cdp, found := k.GetCdpByOwnerAndCollateralType(ctx, owner, collateralType)
	if !found {
		return sdkerrors.Wrapf(types2.ErrCdpNotFound, "owner %s, collateral %s", owner, collateral.Denom)
	}
	deposit, found := k.GetDeposit(ctx, cdp.ID, depositor)
	if !found {
		return sdkerrors.Wrapf(types2.ErrDepositNotFound, "depositor %s, collateral %s %s", depositor, collateral.Denom, collateralType)
	}
	if collateral.Amount.GT(deposit.Amount.Amount) {
		return sdkerrors.Wrapf(types2.ErrInvalidWithdrawAmount, "collateral %s, deposit %s", collateral, deposit.Amount)
	}
	k.hooks.BeforeCDPModified(ctx, cdp)
	cdp = k.SynchronizeInterest(ctx, cdp)
	...
	if collateralizationRatio.LT(liquidationRatio) {
		return sdkerrors.Wrapf(types2.ErrInvalidCollateralRatio, "collateral %s, collateral ratio %s, liquidation ration %s", collateral.Denom, collateralizationRatio, liquidationRatio)
	}

	err = k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types2.ModuleName, depositor, sdk.NewCoins(collateral))
	if err != nil {
		panic(err)
	}
...
func (k Keeper) GetDeposit(ctx sdk.Context, cdpID uint64, depositor sdk.AccAddress) (deposit types2.Deposit, found bool) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.DepositKeyPrefix)
	bz := store.Get(types2.DepositKey(cdpID, depositor))
	if bz == nil {
...
func (k Keeper) SetDeposit(ctx sdk.Context, deposit types2.Deposit) {
	...
	store.Set(types2.DepositKey(deposit.CdpID, deposit.Depositor), bz)
}
...
func (k Keeper) DeleteDeposit(ctx sdk.Context, cdpID uint64, depositor sdk.AccAddress) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.DepositKeyPrefix)
	store.Delete(types2.DepositKey(cdpID, depositor))
}

// IterateDeposits iterates over the all the deposits of a cdp and performs a callback function
func (k Keeper) IterateDeposits(ctx sdk.Context, cdpID uint64, cb func(deposit types2.Deposit) (stop bool)) {
	store := prefix.NewStore(ctx.KVStore(k.key), types2.DepositKeyPrefix)
	iterator := sdk.KVStorePrefixIterator(store, types2.GetCdpIDBytes(cdpID))
...
func (k Keeper) GetDeposits(ctx sdk.Context, cdpID uint64) (deposits types2.Deposits) {
	k.IterateDeposits(ctx, cdpID, func(deposit types2.Deposit) bool {
