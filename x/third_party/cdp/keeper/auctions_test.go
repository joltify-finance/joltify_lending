...
func (suite *AuctionTestSuite) TestNetDebtSurplus() {
	...
	err := bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 100)))
	suite.NoError(err)
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 10)))
	suite.NoError(err)
	suite.NotPanics(func() {
		err := suite.keeper.NetSurplusAndDebt(suite.ctx)
		suite.NoError(err)
	})
	acc := ak.GetModuleAccount(suite.ctx, types2.LiquidatorMacc)
	suite.Equal(cs(c("debt", 90)), bk.GetAllBalances(suite.ctx, acc.GetAddress()))
}

func (suite *AuctionTestSuite) TestCollateralAuction() {
	bk := suite.app.GetBankKeeper()
	err := bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 21000000000), c("bnb", 190000000000)))
	suite.Require().NoError(err)
	testDeposit := types2.NewDeposit(1, suite.addrs[0], c("bnb", 190000000000))
	err = suite.keeper.AuctionCollateral(suite.ctx, types2.Deposits{testDeposit}, "bnb-a", i(21000000000), "usdx")
	suite.Require().NoError(err)
}
...
func (suite *AuctionTestSuite) TestSurplusAuction() {
	...
	err := bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 600000000000)))
	suite.NoError(err)
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 100000000000)))
	suite.NoError(err)
...
func (suite *AuctionTestSuite) TestDebtAuction() {
	...
	err := bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 100000000000)))
	suite.NoError(err)
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 200000000000)))
	suite.NoError(err)
	err = suite.keeper.RunSurplusAndDebtAuctions(suite.ctx)
	suite.NoError(err)
	acc := ak.GetModuleAccount(suite.ctx, auctiontypes.ModuleName)
...
func (suite *AuctionTestSuite) TestGetTotalSurplus() {
	...
	suite.Require().Equal(sdk.NewInt(0), suite.keeper.GetTotalSurplus(suite.ctx, types2.LiquidatorMacc))

	// mint some coins
	err := bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 100e6)))
	suite.Require().NoError(err)
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 200e6)))
	suite.Require().NoError(err)

	// liquidator account has 300e6 total usdx
	suite.Require().Equal(sdk.NewInt(300e6), suite.keeper.GetTotalSurplus(suite.ctx, types2.LiquidatorMacc))

	// mint some debt
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 500e6)))
	suite.Require().NoError(err)
	...
	// burn some usdx
	err = bk.BurnCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 50e6)))
	suite.Require().NoError(err)

	// liquidator usdx decreases
	suite.Require().Equal(sdk.NewInt(250e6), suite.keeper.GetTotalSurplus(suite.ctx, types2.LiquidatorMacc))
}
...
func (suite *AuctionTestSuite) TestGetTotalDebt() {
	...
	// mint some debt
	err := bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 100e6)))
	suite.Require().NoError(err)
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 200e6)))
	suite.Require().NoError(err)

	// liquidator account has 300e6 total debt
	suite.Require().Equal(sdk.NewInt(300e6), suite.keeper.GetTotalDebt(suite.ctx, types2.LiquidatorMacc))

	// mint some usdx
	err = bk.MintCoins(suite.ctx, types2.LiquidatorMacc, cs(c("usdx", 500e6)))
	suite.Require().NoError(err)
	...
	// burn some debt
	err = bk.BurnCoins(suite.ctx, types2.LiquidatorMacc, cs(c("debt", 50e6)))
	suite.Require().NoError(err)

	// liquidator debt decreases
	suite.Require().Equal(sdk.NewInt(250e6), suite.keeper.GetTotalDebt(suite.ctx, types2.LiquidatorMacc))
}
