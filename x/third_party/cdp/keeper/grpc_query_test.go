...
func (suite *grpcQueryTestSuite) TestGrpcQueryParams() {
	...
	suite.tApp.AppCodec().MustUnmarshalJSON(defaultCdpState[types2.ModuleName], &expected)
...
func (suite *grpcQueryTestSuite) TestGrpcQueryParams_Default() {
	suite.keeper.SetParams(suite.ctx, types2.DefaultParams())
...
func (suite *grpcQueryTestSuite) TestGrpcQueryAccounts() {
	...
	liquidator := ak.GetModuleAccount(suite.ctx, types2.LiquidatorMacc)
...
func (suite *grpcQueryTestSuite) TestGrpcQueryTotalPrincipal() {
	...
	res, err := suite.queryServer.TotalPrincipal(sdk.WrapSDKContext(suite.ctx), &types2.QueryTotalPrincipalRequest{})
	suite.Require().NoError(err)

	suite.Len(res.TotalPrincipal, 4, "total principal should include all collateral params")

	suite.Contains(res.TotalPrincipal, types2.TotalPrincipal{
...
func (suite *grpcQueryTestSuite) TestGrpcQueryTotalCollateral() {
	...
	res, err := suite.queryServer.TotalCollateral(sdk.WrapSDKContext(suite.ctx), &types2.QueryTotalCollateralRequest{})
	suite.Require().NoError(err)

	suite.Len(res.TotalCollateral, 4, "total collateral should include all collateral params")
	suite.Contains(res.TotalCollateral, types2.TotalCollateral{
		CollateralType: "xrp-a",
...
func (suite *grpcQueryTestSuite) TestGrpcQueryCdps() {
	...
	res, err := suite.queryServer.Cdps(sdk.WrapSDKContext(suite.ctx), &types2.QueryCdpsRequest{
		CollateralType: "xrp-a",
...
func (suite *grpcQueryTestSuite) TestGrpcQueryCdps_InvalidCollateralType() {
	...
	_, err := suite.queryServer.Cdps(sdk.WrapSDKContext(suite.ctx), &types2.QueryCdpsRequest{
		CollateralType: "joltify-a",
	})
...
func (suite *grpcQueryTestSuite) TestGrpcQueryCdp() {
	...
	tests := []struct {
		giveName     string
		giveRequest  types2.QueryCdpRequest
		wantAccepted bool
		wantErr      string
	}{
		{
			"valid",
			types2.QueryCdpRequest{
				CollateralType: "xrp-a",
		...
		},
		{
			...
			false,
			"joltify-a: invalid collateral for input collateral type",
		},
		{
			"missing owner",
			types2.QueryCdpRequest{
...
func (suite *grpcQueryTestSuite) TestGrpcQueryDeposits() {
	...
	tests := []struct {
		...
		wantContainsDeposit *types2.Deposit
		wantShouldErr       bool
		wantErr             string
	}{
		{
