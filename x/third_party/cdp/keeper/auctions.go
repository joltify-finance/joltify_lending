...
func (k Keeper) AuctionCollateral(ctx sdk.Context, deposits types2.Deposits, collateralType string, debt sdk.Int, bidDenom string) error {
...
func (k Keeper) CreateAuctionsFromDeposit(
	...
	for i := int64(0); i < numAuctions; i++ {
		...
		_, err := k.auctionKeeper.StartCollateralAuction(
			ctx, types2.LiquidatorMacc, sdk.NewCoin(collateral.Denom, auctionSize),
	...
	_, err := k.auctionKeeper.StartCollateralAuction(
		ctx, types2.LiquidatorMacc, sdk.NewCoin(collateral.Denom, lastAuctionCollateral),
...
func (k Keeper) NetSurplusAndDebt(ctx sdk.Context) error {
	totalSurplus := k.GetTotalSurplus(ctx, types2.LiquidatorMacc)
	debt := k.GetTotalDebt(ctx, types2.LiquidatorMacc)
	...
	err := k.bankKeeper.BurnCoins(ctx, types2.LiquidatorMacc, sdk.NewCoins(sdk.NewCoin(k.GetDebtDenom(ctx), netAmount)))
	if err != nil {
		return err
	}
	...
	liquidatorAcc := k.accountKeeper.GetModuleAccount(ctx, types2.LiquidatorMacc)
	balance := k.bankKeeper.GetBalance(ctx, liquidatorAcc.GetAddress(), dp.Denom).Amount
	burnAmount := sdk.MinInt(balance, netAmount)
	return k.bankKeeper.BurnCoins(ctx, types2.LiquidatorMacc, sdk.NewCoins(sdk.NewCoin(dp.Denom, burnAmount)))
}
...
func (k Keeper) RunSurplusAndDebtAuctions(ctx sdk.Context) error {
	...
	remainingDebt := k.GetTotalDebt(ctx, types2.LiquidatorMacc)
	...
	if remainingDebt.GTE(params.DebtAuctionThreshold) {
		...
		_, err := k.auctionKeeper.StartDebtAuction(ctx, types2.LiquidatorMacc, bidCoin, initialLot, debtLot)
		if err != nil {
			return err
		}
	}
	...
	_, err := k.auctionKeeper.StartSurplusAuction(ctx, types2.LiquidatorMacc, surplusLot, k.GetGovDenom(ctx))
	return err
}
