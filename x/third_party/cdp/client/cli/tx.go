...
`, version.AppName, types2.ModuleName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types2.NewMsgCreateCDP(clientCtx.GetFromAddress(), collateral, debt, args[2])
...
Example:
$ %s tx %s deposit jolt1pm9kvrl64fqgxvym7f7m42dndjk52v9mqnzdnn 10000000uatom atom-a --from myKeyName
`, version.AppName, types2.ModuleName)),
		Args: cobra.ExactArgs(3),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types2.NewMsgDeposit(owner, clientCtx.GetFromAddress(), collateral, args[2])
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
...
Example:
$ %s tx %s withdraw jolt1pm9kvrl64fqgxvym7f7m42dndjk52v9mqnzdnn 10000000uatom atom-a --from myKeyName
`, version.AppName, types2.ModuleName)),
		Args: cobra.ExactArgs(3),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types2.NewMsgWithdraw(owner, clientCtx.GetFromAddress(), collateral, args[2])
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
...
`, version.AppName, types2.ModuleName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types2.NewMsgDrawDebt(clientCtx.GetFromAddress(), args[0], debt)
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
...
`, version.AppName, types2.ModuleName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types2.NewMsgRepayDebt(clientCtx.GetFromAddress(), args[0], payment)
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
...
Example:
$ %s tx %s liquidate jolt1pm9kvrl64fqgxvym7f7m42dndjk52v9mqnzdnn btcb-a --from myKeyName
`, version.AppName, types2.ModuleName)),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types2.NewMsgLiquidate(clientCtx.GetFromAddress(), addr, args[1])
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
