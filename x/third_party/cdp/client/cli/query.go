...
Example:
$ %s query %s cdp jolt1pm9kvrl64fqgxvym7f7m42dndjk52v9mqnzdnn atom-a
`, version.AppName, types2.ModuleName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.Cdp(context.Background(), &types2.QueryCdpRequest{
				Owner:          args[0],
...
$ kvcli q cdp cdps --owner=jolt1pm9kvrl64fqgxvym7f7m42dndjk52v9mqnzdnn
...
Example:
$ %s query %s deposits jolt1pm9kvrl64fqgxvym7f7m42dndjk52v9mqnzdnn atom-a
`, version.AppName, types2.ModuleName)),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			...
			res, err := queryClient.Deposits(context.Background(), &types2.QueryDepositsRequest{
				Owner:          args[0],
				CollateralType: args[1],
			})
			if err != nil {
...
func QueryGetAccounts() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.Accounts(context.Background(), &types2.QueryAccountsRequest{})
			if err != nil {
				return err
			}
