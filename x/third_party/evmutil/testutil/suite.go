...
func (suite *Suite) SetupTest() {
	...
	evmGenesis.Params.EvmDenom = "ajolt"
	...
	coins := sdk.NewCoins(sdk.NewInt64Coin("ujolt", 1000_000_000_000_000_000))
	...
	suite.App.InitializeFromGenesisStates(nil, coins, authGS, gs)
...
func (suite *Suite) FundAccountWithJolt(addr sdk.AccAddress, coins sdk.Coins) {
	ujolt := coins.AmountOf("ujolt")
	if ujolt.IsPositive() {
		err := suite.App.FundAccount(suite.Ctx, addr, sdk.NewCoins(sdk.NewCoin("ujolt", ujolt)))
		suite.Require().NoError(err)
	}
	ajolt := coins.AmountOf("ajolt")
	if ajolt.IsPositive() {
		err := suite.Keeper.SetBalance(suite.Ctx, addr, ajolt)
		suite.Require().NoError(err)
	}
}

func (suite *Suite) FundModuleAccountWithJolt(moduleName string, coins sdk.Coins) {
	ujolt := coins.AmountOf("ujolt")
	if ujolt.IsPositive() {
		err := suite.App.FundModuleAccount(suite.Ctx, moduleName, sdk.NewCoins(sdk.NewCoin("ujolt", ujolt)))
		suite.Require().NoError(err)
	}
	ajolt := coins.AmountOf("ajolt")
	if ajolt.IsPositive() {
...
func (suite *Suite) DeployERC20() types.InternalEVMAddress {
	...
	suite.App.FundModuleAccount(
		suite.Ctx,
		types.ModuleName,
		sdk.NewCoins(sdk.NewCoin("ujolt", sdkmath.NewInt(0))),
	)
...
func (suite *Suite) SendTx(
	...
	nonce := evmKeeper.GetNonce(suite.Ctx, suite.Address)
	...
	rsp, err := evmKeeper.EthereumTx(ctx, ercTransferTx)
	if err != nil {
		return nil, err
	}
