...
func NewConversionPair(address InternalEVMAddress, denom string) ConversionPair {
	return ConversionPair{
		JoltERC20Address: address.Address.Bytes(),
		Denom:            denom,
	}
}

// GetAddress returns the InternalEVMAddress of the Jolt ERC20 address.
func (pair ConversionPair) GetAddress() InternalEVMAddress {
	return NewInternalEVMAddress(common.BytesToAddress(pair.JoltERC20Address))
}
...
func (pair ConversionPair) Validate() error {
	...
	if len(pair.JoltERC20Address) != common.AddressLength {
		return fmt.Errorf("address length is %v but expected %v", len(pair.JoltERC20Address), common.AddressLength)
	}

	if bytes.Equal(pair.JoltERC20Address, common.Address{}.Bytes()) {
		return fmt.Errorf("address cannot be zero value %v", hex.EncodeToString(pair.JoltERC20Address))
	}
...
func (pairs ConversionPairs) Validate() error {
	...
	for _, pair := range pairs {
		if addrs[hex.EncodeToString(pair.JoltERC20Address)] {
			return fmt.Errorf(
				"found duplicate enabled conversion pair internal ERC20 address %s",
				hex.EncodeToString(pair.JoltERC20Address),
			)
		}
		...
		addrs[hex.EncodeToString(pair.JoltERC20Address)] = true
