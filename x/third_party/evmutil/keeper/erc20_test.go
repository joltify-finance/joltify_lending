...
func (suite *ERC20TestSuite) TestDeployjoltWrappedCosmosCoinERC20Contract() {
	suite.Run("fails to deploy invalid contract", func() {
		...
		_, err := suite.Keeper.DeployJoltWrappedCosmosCoinERC20Contract(suite.Ctx, invalidToken)
	...
	suite.Run("deploys contract with expected metadata & permissions", func() {
		...
		addr, err := suite.Keeper.DeployJoltWrappedCosmosCoinERC20Contract(suite.Ctx, token)
		suite.NoError(err)
		suite.NotNil(addr)
