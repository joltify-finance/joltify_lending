...
func (suite *invariantTestSuite) SetupValidState() {
	...
	suite.FundModuleAccountWithJolt(
		types.ModuleName,
		sdk.NewCoins(
			sdk.NewCoin("ujolt", sdkmath.NewInt(2)), // ( sum of all minor balances ) / conversion multiplier
		),
	)
...
func (suite *invariantTestSuite) TestSmallBalances() {
	...
	// add same number of ujolt to avoid breaking other invariants
...
func (suite *invariantTestSuite) TestSendToModuleAccountNotAllowed() {
	...
	suite.ErrorContains(err, "jolt1w9vxuke5dz6hyza2j932qgmxltnfxwl73fvt2c is not allowed to receive funds: unauthorized")
}
