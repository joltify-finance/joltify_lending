...
func GetQueryCmd(queryRoute string) *cobra.Command {
	...
	cmd.AddCommand(CmdDepositorHistory())

	cmd.AddCommand(CmdAllowedPools())

	cmd.AddCommand(CmdClaimableInterest())

	cmd.AddCommand(CmdOutstandingInterest())

	cmd.AddCommand(CmdwithdrawalPrincipal())

	cmd.AddCommand(CmdTotalReserve())

	cmd.AddCommand(CmdPoolInvestors())
