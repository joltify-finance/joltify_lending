// Generated by Ignite ignite.com/cli

import { StdFee } from "@cosmjs/launchpad";
import { SigningStargateClient, DeliverTxResponse } from "@cosmjs/stargate";
import { EncodeObject, GeneratedType, OfflineSigner, Registry } from "@cosmjs/proto-signing";
import { msgTypes } from './registry';
import { IgniteClient } from "../client"
import { MissingWalletError } from "../helpers"
import { Api } from "./rest";
import { MsgRedeemTokens } from "./types/joltify/issuance/v1beta1/tx";
import { MsgSetPauseStatus } from "./types/joltify/issuance/v1beta1/tx";
import { MsgBlockAddress } from "./types/joltify/issuance/v1beta1/tx";
import { MsgUnblockAddress } from "./types/joltify/issuance/v1beta1/tx";
import { MsgIssueTokens } from "./types/joltify/issuance/v1beta1/tx";


export { MsgRedeemTokens, MsgSetPauseStatus, MsgBlockAddress, MsgUnblockAddress, MsgIssueTokens };

type sendMsgRedeemTokensParams = {
  value: MsgRedeemTokens,
  fee?: StdFee,
  memo?: string
};

type sendMsgSetPauseStatusParams = {
  value: MsgSetPauseStatus,
  fee?: StdFee,
  memo?: string
};

type sendMsgBlockAddressParams = {
  value: MsgBlockAddress,
  fee?: StdFee,
  memo?: string
};

type sendMsgUnblockAddressParams = {
  value: MsgUnblockAddress,
  fee?: StdFee,
  memo?: string
};

type sendMsgIssueTokensParams = {
  value: MsgIssueTokens,
  fee?: StdFee,
  memo?: string
};


type msgRedeemTokensParams = {
  value: MsgRedeemTokens,
};

type msgSetPauseStatusParams = {
  value: MsgSetPauseStatus,
};

type msgBlockAddressParams = {
  value: MsgBlockAddress,
};

type msgUnblockAddressParams = {
  value: MsgUnblockAddress,
};

type msgIssueTokensParams = {
  value: MsgIssueTokens,
};


export const registry = new Registry(msgTypes);

const defaultFee = {
  amount: [],
  gas: "200000",
};

interface TxClientOptions {
  addr: string
	prefix: string
	signer?: OfflineSigner
}

export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {

  return {
		
		async sendMsgRedeemTokens({ value, fee, memo }: sendMsgRedeemTokensParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgRedeemTokens: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgRedeemTokens({ value: MsgRedeemTokens.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgRedeemTokens: Could not broadcast Tx: '+ e.message)
			}
		},
		
		async sendMsgSetPauseStatus({ value, fee, memo }: sendMsgSetPauseStatusParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgSetPauseStatus: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgSetPauseStatus({ value: MsgSetPauseStatus.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgSetPauseStatus: Could not broadcast Tx: '+ e.message)
			}
		},
		
		async sendMsgBlockAddress({ value, fee, memo }: sendMsgBlockAddressParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgBlockAddress: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgBlockAddress({ value: MsgBlockAddress.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgBlockAddress: Could not broadcast Tx: '+ e.message)
			}
		},
		
		async sendMsgUnblockAddress({ value, fee, memo }: sendMsgUnblockAddressParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgUnblockAddress: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgUnblockAddress({ value: MsgUnblockAddress.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgUnblockAddress: Could not broadcast Tx: '+ e.message)
			}
		},
		
		async sendMsgIssueTokens({ value, fee, memo }: sendMsgIssueTokensParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgIssueTokens: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgIssueTokens({ value: MsgIssueTokens.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgIssueTokens: Could not broadcast Tx: '+ e.message)
			}
		},
		
		
		msgRedeemTokens({ value }: msgRedeemTokensParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.issuance.v1beta1.MsgRedeemTokens", value: MsgRedeemTokens.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgRedeemTokens: Could not create message: ' + e.message)
			}
		},
		
		msgSetPauseStatus({ value }: msgSetPauseStatusParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.issuance.v1beta1.MsgSetPauseStatus", value: MsgSetPauseStatus.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgSetPauseStatus: Could not create message: ' + e.message)
			}
		},
		
		msgBlockAddress({ value }: msgBlockAddressParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.issuance.v1beta1.MsgBlockAddress", value: MsgBlockAddress.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgBlockAddress: Could not create message: ' + e.message)
			}
		},
		
		msgUnblockAddress({ value }: msgUnblockAddressParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.issuance.v1beta1.MsgUnblockAddress", value: MsgUnblockAddress.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgUnblockAddress: Could not create message: ' + e.message)
			}
		},
		
		msgIssueTokens({ value }: msgIssueTokensParams): EncodeObject {
			try {
				return { typeUrl: "/joltify.issuance.v1beta1.MsgIssueTokens", value: MsgIssueTokens.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgIssueTokens: Could not create message: ' + e.message)
			}
		},
		
	}
};

interface QueryClientOptions {
  addr: string
}

export const queryClient = ({ addr: addr }: QueryClientOptions = { addr: "http://localhost:1317" }) => {
  return new Api({ baseURL: addr });
};

class SDKModule {
	public query: ReturnType<typeof queryClient>;
	public tx: ReturnType<typeof txClient>;
	
	public registry: Array<[string, GeneratedType]>;

	constructor(client: IgniteClient) {		
	
		this.query = queryClient({ addr: client.env.apiURL });
		this.tx = txClient({ signer: client.signer, addr: client.env.rpcURL, prefix: client.env.prefix ?? "cosmos" });
	}
};

const Module = (test: IgniteClient) => {
	return {
		module: {
			JoltifyIssuanceV1Beta1: new SDKModule(test)
		},
		registry: msgTypes
  }
}
export default Module;